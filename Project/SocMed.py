...
def likes_posting():
    ...
    cek=lkspostby.query.filter_by(idposting=data["idposting"]).filter_by(likeby=data["usrid"]).first()
    lksby = lkspostby(
    ...
    if cek:
        return {
            "Message": "Sudah Pernah Menyukai Postingan ini"
        }, 400
...
#UNFOLLOW USER
@app.route('/fldlte', methods=['DELETE'])
def dlte_flwr():
    ...
    flwr = follower.query.filter_by(usrid=data['usrid']).filter_by(followerid=data['followerid']).first()

    # return str(flwr)
    try:
        # flwr = follower.query.filter_by(usrid=usrid).filter_by(followerid=followerid).first()
        db.session.delete(flwr)
        db.session.commit()
    except:
        return {
...
def dlte_psting(idpost):
    ...
    if str(cekusrpost.idposting==idpost):
        try:
        ...
        return {
            "Message": "Hapus Belum Berhasil,Data Postingan Tidak Ditemukan"
        },404
#UPDATE DATA USER
@app.route('/updtusr/<id>', methods=['PUT'])
def update_usr(id):
    # parsed = BasicAuth()
    # username = parsed[0]
    # password= parsed[1]
    
    cek=user.query.filter_by(usrid=id).first()
...
@app.route('/whoto/<id>', methods=['GET'])
def get_whoto(id):
    d=db.engine.execute('SELECT * FROM public.user WHERE usrid NOT IN (SELECT usrid FROM public.follower WHERE followerid='+str(id)+')limit 3')
    # flwr = follower.query.filter_by(usrid=id).filter_by(apprvl=1).all()
    arr=[]
    listname=[]
    if d:
        for i in d:
            if i[0]!=int(id):
                arr.append({'usrid':i[0],'usrname':i[1],'nick':i[3]})
...
def get_usr():
    ...
    return jsonify(arr)
#GetTotalUsr
@app.route('/ttlusr', methods=['GET'])
def get_ttlusr():
    a=db.engine.execute('SELECT * FROM public.user')
...
#GetTotalTweet
@app.route('/ttltweet', methods=['GET'])
def get_ttlpst():
    a=db.engine.execute('SELECT * FROM public.posting')
...
#GETUSR FOR SETTING
@app.route('/settusr/<id>', methods=['GET'])
def get_settusr(id):
    a=db.engine.execute('SELECT usrid,usrname,password,nickname FROM public.user WHERE usrid='+str(id)+'')
    
    arr=[]
    for i in a:
        flwr = follower.query.filter_by(usrid=i[0]).filter_by(apprvl=1).count()
        flwing=follower.query.filter_by(followerid=i[0]).filter_by(apprvl=1).count()
        arr.append({'usrid':i[0],'usrname':i[1],'pass':i[2],'nick':i[3]})

    return jsonify(arr)



@app.route('/getusrbyid/<id>', methods=['GET'])
def get_usrbyid(id):
    a=db.engine.execute('SELECT usrid,usrname,nickname,lastlogin FROM public.user WHERE usrid='+str(id)+'')
    
    arr=[]
    for i in a:
        flwr = follower.query.filter_by(usrid=i[0]).filter_by(apprvl=1).count()
        flwing=follower.query.filter_by(followerid=i[0]).filter_by(apprvl=1).count()
        arr.append({'usrid':i[0],'Nickname':i[2],'LastLogin':i[3],'Follower':flwr,'Following':flwing,'usrname':i[1]})
...
@app.route('/getalltweet', methods=['GET'])
def all_tweet():
    post = posting.query.all()
    arr=[]
    # usrname=user.query.filter_by(usrid=1).all()

    # return {'id':usrname.nickname}
    for i in post:
        a=db.engine.execute('SELECT COUNT(likeby) FROM public.lkspostby WHERE idposting='+str(i.idposting)+'')
        usrname=user.query.filter_by(usrid=i.usrid).first()
        for j in a:
            arr.append({'idposting':i.idposting,'Tweet':i.isipost,'username':usrname.usrname,'nick':usrname.nickname,'userid':usrname.usrid,'TotalLikes':j[0]})
    
    return jsonify(arr)


#CEK LIKES
@app.route('/ceklikes/<idpst>/<idusr>', methods=['GET'])
def cek_likes(idpst,idusr):
#    return idpst
    cek=lkspostby.query.filter_by(idposting=idpst).filter_by(likeby=idusr).first()
    arr=[]
    if cek:
        arr.append({'stslikes':"Ada"})
    else:
        
        arr.append({'stslikes':"BelumAda"})
...
def list_tweet(id):
    ...
    arr=[]
    for i in post:
        cekidpost=i.idposting
        # return str(cekidpost)
        a=db.engine.execute('SELECT COUNT(likeby) FROM public.lkspostby WHERE idposting='+str(cekidpost)+'')
        for j in a:
            # return str(j[0])
            arr.append({'idposting':i.idposting,'Tweet':i.isipost,'TtlLikes':j[0]})
...
def get_mostpopusr():
    a=db.engine.execute('SELECT * FROM(SELECT usrid as id,COUNT(usrid) as id_count FROM public.follower GROUP BY usrid)t ORDER BY id_count desc limit 10')
    arr=[]
    for i in a:
        ncknme = user.query.filter_by(usrid=i[0]).first()
        
        arr.append({'usrid':i[0],'Nickname':str(ncknme.nickname),'Followers':i[1],'Usernme':str(ncknme.usrname)})
    return jsonify(arr)

#GET POPULAR TWEET
@app.route('/poptweet', methods=['GET'])
def get_mostpoptweet():
    a=db.engine.execute('SELECT * FROM(SELECT idposting as id,COUNT(idposting) as post_count FROM public.lkspostby GROUP BY idposting)t ORDER BY post_count desc limit 5')
    arr=[]
    for i in a:
        postnme = posting.query.filter_by(idposting=i[0]).first()
        usr = user.query.filter_by(usrid=postnme.usrid).first()
        
        
        arr.append({'idposting':i[0],'IsiPost':str(postnme.isipost),'LikesCount':i[1],'Usernme':str(usr.usrname)})
    return jsonify(arr)
    # return(arr)
#GET POPULAR TWEET
@app.route('/ppstwitindex', methods=['GET'])
def get_mstpoptwit():
    a=db.engine.execute('SELECT * FROM(SELECT idposting as id,COUNT(idposting) as post_count FROM public.lkspostby GROUP BY idposting)t ORDER BY post_count desc limit 3')
    arr=[]
    for i in a:
        postnme = posting.query.filter_by(idposting=i[0]).first()
        postby=user.query.filter_by(usrid=postnme.usrid).first()
        arr.append({'idposting':i[0],'IsiPost':str(postnme.isipost),'LikesCount':i[1],'nick':str(postby.nickname)})
    return jsonify(arr)
